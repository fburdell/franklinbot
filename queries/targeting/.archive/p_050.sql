delete from demspahdcc.commons.hdcc_persuasion_unsurpressed_target where dist_sth = '050'; --target administration

insert into demspahdcc.commons.hdcc_persuasion_unsurpressed_target (dist_sth, vanid) --more target administration

select vf.dist_sth, targ.vanid --select data for target administration
...
and vf.party <> 'D'
) seg_e --civis add, really tight in HD050 here, civis picks up a lot of people in the .1-.5 partisanship range. until we get a better idea of how scores are benchmarking this year, i want to keep both of these applied equally for now
...
where 
  safe_cast(vf.dist_sth as int64) in (50) --check if hd in tiered hd
